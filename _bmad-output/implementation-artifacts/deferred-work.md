...
## Story 6.4 → OI-11 real-sink hardening requirements (audit)
The security/privacy/reliability/observability audit (`security-privacy-reliability-observability-verification.md`) confirmed the spike's cross-cutting invariants are enforced in-harness (PII boundary, no-PII analytics, consent gate, encryption/retention markings, requestId), and surfaced the following **real-sink** requirements for OI-11 (real CRM/backend/deploy), none of which are spike defects: (1) **real at-rest encryption** backing the `encryptAtRest:true` marking; (2) **enforced 24-month retention** backing `retentionMonths:24` (purge job); (3) **bounded/TTL idempotency ledger** + shared multi-instance store (subsumes the Story 5.4 unbounded-ledger item); (4) **real rate-limiter** for NFR-7 (address 100/min, estimate 50/min, lead 20/min + 30s timeout) at the gateway/middleware seam — none exists in the spike; (5) **real transport security** for NFR-5 (TLS 1.2+, API-key auth, CORS to `*.demo.channel.com`) at deploy (`HANDOVER_05`); (6) optional **payload-aware idempotency** (subsumes Story 5.4 EH#5) if required downstream.
